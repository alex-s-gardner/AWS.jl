...
Gets the token balance for a batch of tokens by using the GetTokenBalance action for every
token in the request.  Only the native tokens BTC,ETH, and the ERC-20, ERC-721, and ERC
1155 token standards are supported.

# Optional Parameters
Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:
- `"getTokenBalanceInputs"`: An array of GetTokenBalanceInput objects whose balance is
  being requested.
"""
...
This action returns the following for a given a blockchain network:   Lists all token
balances owned by an address (either a contact address or a wallet address).   Lists all
