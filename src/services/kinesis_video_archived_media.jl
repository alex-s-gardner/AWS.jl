...
- `"SamplingInterval"`: The time interval in milliseconds (ms) at which the images need to
  be generated from the stream, with a default of 3000 ms. The minimum value that can be
  provided is 200 ms. If the timestamp range is less than the sampling interval, the Image
  from the startTimestamp will be returned if available.   The minimum value of 200 ms is a
  hard limit.
