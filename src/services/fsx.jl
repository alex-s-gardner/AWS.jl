...
archived to S3. The metadata of released files remains on the file system so users or
...
- `type`: Specifies the type of data repository task to create.    EXPORT_TO_REPOSITORY
  ...
  in your Amazon FSx for Lustre file system that are archived and that meet your specified
  release criteria.    AUTO_RELEASE_DATA tasks automatically release files from an Amazon
  File Cache resource.
...
- `"Paths"`: A list of paths for the data repository task to use when the task is
  ...
  files from S3 (for import tasks), or release all archived files that meet the last accessed
  ...
  from which to release archived files. If a directory is specified, files within the
  directory are released. If a file path is specified, only that file is released. To release
  all archived files in the file system, specify a forward slash (/) as the path.  A file
...
LogConfiguration     LustreRootSquashConfiguration     StorageCapacity
WeeklyMaintenanceStartTime    For FSx for ONTAP file systems, you can update the following
properties:    AddRouteTableIds     AutomaticBackupRetentionDays
